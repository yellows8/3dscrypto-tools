...
static int dltitle = 0, packcia = 0, dectitle = 0, disasm_title = 0;
static int noromfs = 0;
int disablencch = 0;
...
int settings_load()
{
	...
	if(!settingspath_set)
	{
		...
		snprintf(settingspath, sizeof(settingspath)-1, "%s/.3ds/ctr-titletool_settings", strptr);
	}
...
int process_ncch(u32 contentid, char *titlepath)
{
	int ret;
	char sys_cmd[1024];
	char basepath[1024];
	...
	snprintf(sys_cmd, sizeof(sys_cmd)-1, "ctr-ncchtool --input=%s.app --output=%s.bin --ctrtoolprefix=%s %s %s %s", basepath, basepath, basepath, noromfs_cmd, serveradr_cmd, disasm_cmd);
	#ifndef EMBED_NCCHTOOL
	printf("Running command: %s\n", sys_cmd);
	ret = system(sys_cmd);
	#else
	printf("Running the embedded ctr-ncchtool: %s\n", sys_cmd);

	strptr = strtok(sys_cmd, " ");
	while(strptr)
	{
		argv_[argpos] = strptr;
		argpos++;
...
int decrypt_contents(uint64_t titleid, char *titlepath, ctr_tmd_contentchunk *chunks, u16 total_contents)
{
	...
	{
		printf("Skipping NCCH processing since this is a TWL title.\n");
	}
	else if(disablencch==0)
	{
		for(index=0; index<total_contents; index++)
		{
			...
			ret = process_ncch(contentid, titlepath);
			if(ret!=0)
			{
				printf("Failed to process contentID %08x NCCH.\n", contentid);
				return ret;
			}
		}
	}
...
int main(int argc, char *argv[])
{
	...
	int argc_ = 0;
	char *argv_[32] = {0};
	#ifdef DEFAULT_ADDITIONAL_ARGS
	char additional_args[1024] = {DEFAULT_ADDITIONAL_ARGS};
	#endif
	...
	if(argc==1)
	{
		...
		printf("--noromfs Pass the --noromfs option to ctr-ncchtool.\n");
		printf("--disasm Pass the --disasm option to ctr-ncchtool.\n");
		...
		printf("--decrypt=titlekey Decrypt the title stored in titledir, and use ctr-ncchtool to decrypt the NCCH contents. When '=<titlekey>' is specified, use the input titlekey to decrypt content instead of decrypting the tik titlekey. Otherwise, when the settings file was successfully loaded, the plaintext titlekey from there will be used for the title if available.\n");
		printf("--decwithcommonkey=path When --decrypt is used, load normal commonkey from path.\n");
		printf("--disablencch Don't run ctr-ncchtool when --decrypt was used.\n");
	...
	for(argi=1; argi<argc_+argc; argi++)
	{
		if(argi-1 >= argc_)
		{
			argptr = argv[argi-argc_];
		}
		else
		{
			argptr = argv_[argi-1];
		...
		if(strncmp(argptr, "--serveradr=", 12)==0)
		{
			serveradr_set = 1;
			strncpy(serveradr, &argptr[12], 255);
		}

		if(strncmp(argptr, "--tik=", 6)==0)
		{
			tikdecrypt_titlekey(&argptr[6], titlekey);
		}

		if(strncmp(argptr, "--dltitle", 9)==0)
		{
			dltitle = 1;
		}
		if(strncmp(argptr, "--packcia", 9)==0)
		{
			packcia = 1;
		}
		if(strncmp(argptr, "--noromfs", 9)==0)
		{
			noromfs = 1;
		}
		if(strncmp(argptr, "--disasm", 8)==0)
		{
			disasm_title = 1;
		}

		if(strncmp(argptr, "--titleid=", 10)==0)
		{
			if(strlen(&argptr[10]) != 16)
			{
			...
			{
				titleid_set = 1;
				sscanf(&argptr[10], "%016"PRIx64, &titleid);
			}
		}

		if(strncmp(argptr, "--titlepath=", 12)==0)strncpy(titlepath, &argptr[12], 255);

		if(strncmp(argptr, "--titlever=", 11)==0)
		{
			titleversion_set = 1;
			sscanf(&argptr[11], "%u", &titleversion);
		}

		if(strncmp(argptr, "--decrypt", 9)==0)
		{
			dectitle = 1;
			if(strlen(argptr) == 10+32)
			{
				for(i=0; i<16; i++)
				{
					sscanf(&argptr[10 + i*2], "%02x", &tmp);
					intitlekey[i] = tmp;
				}
		...
		if(strncmp(argptr, "--disablencch", 13)==0)
		{
			disablencch = 1;
		}

		if(strncmp(argptr, "--decwithcommonkey=", 19)==0)
		{
			strncpy(commonkey_path, &argptr[19], sizeof(commonkey_path)-1);
		}

		if(strncmp(argptr, "--csv", 5)==0)
		{
			use_csv = 1;
			if(argptr[5] == '=')strncpy(csvpath, &argptr[6], 255);
		}

		if(strncmp(argptr, "--firstvercsv", 13)==0)csv_versionhandling = 1;
		if(strncmp(argptr, "--lastvercsv", 12)==0)csv_versionhandling = 2;
		if(strncmp(argptr, "--disabletitledups", 18)==0)disabletitledups = 1;
		if(strncmp(argptr, "--disablemultiregion", 20)==0)disablemultiregion = 1;
		if(strncmp(argptr, "--disablecsvtitledir", 20)==0)disablecsvtitledir = 1;
		if(strncmp(argptr, "--enablecsvtitleretry", 21)==0)enable_csvtitle_retry = 1;
		if(strncmp(argptr, "--csvignore404", 14)==0)csvignore404 = 1;

		if(strncmp(argptr, "--begintitle=", 13)==0)
		{
			if(strlen(&argptr[13]) != 16)
			{
				printf("Invalid titleID.\n");
			}
			else
			{
				sscanf(&argptr[13], "%016"PRIx64, &begintitleid);
			}
		}

		if(strncmp(argptr, "--disablesettings", 17)==0)
		{
			enable_settingsloading = 0;
		}

		if(strncmp(argptr, "--settingspath=", 15)==0)
		{
			settingspath_set = 1;
			memset(settingspath, 0, sizeof(settingspath));
			strncpy(settingspath, &argptr[15], sizeof(settingspath)-1);
		}
	}
